...
public class AmazonPrimeAirRoute {
  ...
  */
    ...
    for (int[] b : B) {
      int remainingTarget = target - b[1];
      // We will find the remaining target by the value from the value of B.
      ...
      if (valueWithIds.containsKey(remainingTarget)) // That means we find the closest target == currentTarget
      {
        ...
        Integer floor = valueWithIds.floorKey(remainingTarget);
        // Check if we have any least value which is less than remainingTarget.
