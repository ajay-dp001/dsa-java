...
public class MaxSumBtwTwoLeaves {

  private static int maxSumBtwTwoLeaf = 0;
  ...
  private static int maxPathSum(Node node) {
    ...
    if (node.left != null && node.right != null) {
      maxSumBtwTwoLeaf = Math.max(maxSumBtwTwoLeaf, ls + rs + node.key);
      return (Math.max(ls, rs) + node.key);
    }
    return node.left != null ? ls + node.key : rs + node.key;
  }
